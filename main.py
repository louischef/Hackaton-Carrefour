...
Ta tâche est de retrouver toutes les marques appartenant à un groupe (holding) donné.

Pour chaque marque que tu identifies, retourne une seule ligne au format :
nom_de_la_marque:score_de_certitude

Le score de certitude est un nombre compris entre **0** (aucune certitude) et **1** (certitude absolue, confirmée par source officielle), avec possibilité d’utiliser des paliers intermédiaires comme 0.9, 0.7, 0.5, etc.

Critères :
- Score **1** : la marque figure explicitement sur le site officiel du groupe ou dans ses documents financiers récents.
- Score **0.9** : la marque est mentionnée dans une source fiable proche du groupe (rapport d'une filiale, dépôt officiel, etc.).
- Score **0.7 ou moins** : la relation est probable mais non confirmée directement.
- Score **0.3 ou moins** : présence ancienne ou indirecte, doute raisonnable.
- Score **0** : données contradictoires ou impossibles à vérifier.
- `NaN` : aucune information trouvée ou rien de pertinent.

Règles :
- N’utilise que des **sources officielles, institutionnelles ou vérifiables** (rapports annuels, sites corporate, bases de données d’entreprises, publications PDF).
...
def add_new_brands_to_dataframe(df, new_brands_data):
    ...
    existing_brands = set()
    for _, row in df.iterrows():
        existing_brands.add((row['Main Holding Name'], row['Brand Name']))
    ...
    for holding, brands in new_brands_data.items():
        ...
        for brand_data in brands:
            ...
            # Vérifier si la marque n'existe pas déjà
            if (holding, brand_name) not in existing_brands:
                ...
                print(f"[DEBUG] Marque déjà existante ignorée: {brand_name}")
                ...
                for line in content.strip().split('\n'):
                    if ':' in line:
                        ...
                        try:
                            ...
                            # Vérifier que la marque n'est pas déjà dans les marques existantes
                            if brand not in existing_brands and confidence >= 0.5:  # Seuil minimum de confiance
                                print(f"[DEBUG] Nouvelle marque trouvée: {brand} (confiance: {confidence}, secteur: {sector}, département: {department})")
                                ...
                                print(f"[DEBUG] Marque ignorée (existante ou confiance faible): {brand}")
